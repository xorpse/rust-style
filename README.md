...
- `thiserror_conventions` — `pub` error types must not be named `Error`; variants of `thiserror` enums must be alphabetical; `*Error` types must derive `thiserror::Error`; complex variants without `#[from]` need a snake-case constructor.
